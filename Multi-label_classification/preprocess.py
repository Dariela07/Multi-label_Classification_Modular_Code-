...
def get_input_data() -> pd.DataFrame:
    ...
    # Create dependent variable for each class column
    for c in Config.CLASS_COL:
        df = df.loc[(df[c] != '') & (~df[c].isna()),]
    return df
...
def noise_remover(df: pd.DataFrame):
    """"
    Removes specified noise patterns and extra whitespace in TICKET_SUMMARY and INTERACTION_CONTENT columns.
    Filters rows based on the value counts in the y1, y2, y3, and y4 columns, keeping those with counts above a specified threshold.
    """
    ...
    good_y2 = df.y2.value_counts()[df.y2.value_counts() > 2].index
    good_y3 = df.y3.value_counts()[df.y3.value_counts() > 2].index
    good_y4 = df.y4.value_counts()[df.y4.value_counts() > 2].index

    df = df.loc[df.y2.isin(good_y2)]
    df = df.loc[df.y3.isin(good_y3)]
    df = df.loc[df.y4.isin(good_y4)]
